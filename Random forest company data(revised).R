...
model<-randomForest(`Sales type` ~.,data= company,ntree= 1000,na.action = na.roughfix)
model
importance(model)
...
plot(model)
hist(treesize(model),main = 'No. of nodes for the trees', col= 'blue')

varImpPlot(model,sort=T,n.var = 8, main = 'Top 8 Important variables')
